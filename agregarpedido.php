			...
			<div class="col-lg-4">
				<div class="row">
					...
					<div class="box">
						...
						<div class="body">
							<div class="row">
							<?php $combos= $db->query("SELECT id, combo, valor, descripcion, imagen 
														FROM tbl_combo where estado= 1; ");	
								while ($comb = $db -> fetch_array($combos)) 
									{
									?>
								<div class="col-lg-3">
							<div class="checkbox">
								
                                <input class="uniform" type="checkbox" value="<?php echo $comb['combo']  ?>" id="checkboxc<?php echo $comb['id']  ?>" onClick="validaCheckboxCombos(<?php echo $comb['id'] ?>,'<?php echo $comb['combo']  ?>',<?php echo $comb['valor'] ?>)" >
								<div class="sombra"></div>
								<label class="checkbox" for="checkboxc<?php echo $comb['id']  ?>" style="width: 100%; height: 120px;">
                                    <img src='<?php echo $comb['imagen']  ?>' height="100" style="position: absolute; left: 0px">
                                </label>
                            </div>
									</div>
					...
					<div class="box">
						...
						<div class="body">
							...
							<div class="checkbox">
                                ...
                                <input class="uniform" type="checkbox" value="<?php echo $prod['nombre']  ?>" id="checkbox<?php echo $prod['id']  ?>" onClick="validaCheckbox(<?php echo $prod['id'] ?>,'<?php echo $prod['nombre']  ?>',<?php echo $prod['valor_venta'] ?>,<?php echo $prod['cantidad'] ?>)" >
								<div class="sombra"></div>
						...
						<div class="body">
							 <form action='agregar.php' method='post' enctype='multipart/form-data' >
								...
								<div class='form-group'>
										...
										echo "<option  value=".$_GET['id'].">".$_GET['mesa']." (Cantidad de puestos ".$_GET['Np'].")</option>";
									}?>
										</select>
								 ...
								 <div class="form-group row">
									<div class="col-lg-6">
										<input type='hidden' name ='tbl' value='pedidos'>										
									 	...
									 	<a href="apedidos.php" class="btn btn-info">Ver Pedidos</a>
...
function validaCheckboxCombos(id,nombre,valor)
{
  var checked = checkboxc1.checked;
  
  if( $('#checkboxc'+id).prop('checked') ) { 
	  
	  $('#contenido').append('<div class="form-group" id="rowc_'+id+'"><label for="autosize" class="control-label col-lg-3">'+nombre+'  Cantidad:</label><div class="col-lg-3"><input class="form-control" type="number" id="producto" min="1" value="1" name="combo_'+id+'" placeholder="Cantidad" onchange="myFunctionC(this.value,'+id+','+valor+')"></div><label for="autosize" class="control-label col-lg-2">Valor:</label><div class="col-lg-3"><input class="form-control" type="number" id="valorc'+id+'" min="1" value="'+valor+'" name="valorc'+id+'"  placeholder="Cantidad" readonly><input type="hidden" name ="idcombo_'+id+'" value="'+id+'"><br></div></div>');
	  var pedidos = [1,id,valor];
	  localStorage.setItem('pedidos'+id,JSON.stringify(pedidos));
	  var total= 0;
		for(var i=0, len=localStorage.length; i<len; i++) {
...
function myFunctionC(val,id,valor) {
    //alert("el valor cambia por: " + val*valor + "del producto"+ id);
	var valorfina = val*valor;
	document.getElementById("valorc"+id).value=valorfina; 
	var pedidos = [val,id,valorfina];
	localStorage.setItem('pedidos'+id,JSON.stringify(pedidos));
	var local = localStorage.length;	
	var total= 0;
